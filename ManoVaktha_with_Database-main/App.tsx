...
const App: React.FC = () => {
  ...
  const handleProblemSubmit = useCallback(async (userProblem: string) => {
    ...
    try {
      ...
      console.log('✅ Solutions fetched successfully:', fetchedSolutions.length);
      
      // Check if we're using mock data
      if (fetchedSolutions.length > 0 && fetchedSolutions[0].title.includes('Wisdom') || fetchedSolutions[0].title.includes('ज्ञान') || fetchedSolutions[0].title.includes('జ్ఞానం')) {
        console.log('🧪 Using mock data for demonstration');
        setError('Using demonstration data. Real AI service will be available soon.');
      }
      ...
      if (fetchedSolutions.length > 0) {
        ...
        console.warn('No solutions returned from API');
        setError(t('noSolutionsFound'));
      }
    } catch (err: any) {
      console.error('💥 Error in handleProblemSubmit:', err);
  ...
  const renderManuscriptContent = () => {
    switch (appState) {
      ...
      case 'error':
        ...
        return (
          <div className={`text-center ${isNetworkError ? 'text-amber-800 bg-amber-100/50 border-amber-500/50' : 'text-red-800 bg-red-100/50'} border-2 rounded-lg p-4 mb-8 animate-fade-in`}>
            <p className="font-bold text-lg">
              {isNetworkError 
                ? t('networkError') || "We're having trouble connecting to our wisdom source. Please check your internet connection and try again. In the meantime, you can explore other sections of the app." 
                : error}
            </p>
            {isNetworkError && (
              <p className="mt-2 text-sm">
                {t('networkErrorSuggestion') || "The app will continue to work with sample content while we resolve this issue."}
              </p>
            )}
          </div>
        );
