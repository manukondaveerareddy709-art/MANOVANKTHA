...
const AnalysisView: React.FC = () => {
    ...
    const handleGetGuidance = async () => {
        ...
        const symptomText = Array.from(selectedSymptoms).map(symptomKey => t(symptomKey as any)).join(', ');
...
Please provide some personalized guidance.`;
        ...
        try {
            ...
            const responseStream = streamChat(historyForAPI, prompt, language);
    ...
    const handleRegularSendMessage = async (e: FormEvent) => {
        ...
        try {
            // Pass the up-to-date message history
            const responseStream = streamChat(historyForAPI, trimmedInput, language);
            for await (const chunk of responseStream) {
                setMessages(prev => {
                    const lastMessage = prev[prev.length - 1];
                    if (lastMessage?.role === 'model') {
